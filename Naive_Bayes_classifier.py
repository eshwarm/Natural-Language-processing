...
def get_class_words_dict():
    ...
    for line in lines:
        ...
        all_words.extend(sentence_list) #storing all words
        class_words_dict[class_name].extend(sentence_list) #storing words per class in a dictionary  
...
def test_naive_bayes(object_dict):
    ...
    print("The Acurracy of Naïve Bayes classifier is",(acc_count/line_count)*100)   #total accuracy 
    

if __name__ == "__main__":
    n_doc = get_class_words_dict() 
    object_dict = collections.defaultdict(object)  
     
    for class_, list_ in class_words_dict.items():
        ob = Classifier(class_, n_doc ) 
        object_dict[class_] = ob
    test_naive_bayes(object_dict)    
